...
resource TaskList(subject: str?) {
  ...
  /** the subject of the task list */
  property subject: str;

  // how do i check if a certain input was passed into the function?
  if prefix? {
    public inflight task_count() {
  ...
  /** 
   * max number of tasks 
   * @default - default is 30 unless prefix is defined and then its based on the length
   **/
  option max ?= prefix + 10;
   ...
   */
  public find_tasks_with(): Array<str> {
    /** the term to search */
    let in term: str;
